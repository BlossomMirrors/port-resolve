...
- **AAC audio**: not supported for import in either version. A typical screen-recording `.mp4` (H.264 video + AAC audio) needs *both* re-encoded, not just the audio — [`shell/convert_for_resolve.sh`](shell/convert_for_resolve.sh) probes each file and only re-encodes what's actually incompatible (video to DNxHR if it isn't already DNxHD/ProRes/etc., audio to PCM if it isn't already PCM): `./convert_for_resolve.sh .mp4`.
...
Resolve doesn't surface these codec errors in its UI. An incompatible import just silently vanishes from the timeline with no preview, with the actual reason only visible in its debug log. [`shell/arc_compat_watcher.sh`](shell/arc_compat_watcher.sh) (named "Arc Compat") is launched automatically alongside Resolve by `resolve.sh` and sends a desktop notification (via the Flatpak portal, so it works on any desktop environment) the moment Resolve logs one of these errors, with the exact `flatpak run --command=convert-for-resolve` invocation to fix that file, plus an "Open Docs" button, instead of having to dig through `~/.var/app/com.blackmagic.Resolve/data/logs/ResolveDebug.txt` yourself.
